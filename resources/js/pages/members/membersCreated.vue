    ...
    <v-dialog v-model="dialogUpload" width="500">
      <v-card>
        ...
        <v-card-text>
          ...
          <img :src="form.fotoBase64">
          <div class="size">Output size: {{form.fotoBase64.length}}</div>
        </v-card-text>
        <v-divider></v-divider>
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn color="primary" flat @click="form.fotoBase64 = myCroppa.generateDataUrl('image/jpeg', 0.8)">
            Salvar imagem
          </v-btn>
        </v-card-actions>
      </v-card>
